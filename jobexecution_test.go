...
func TestJobExecutionUpdatesWithOptionalParams(t *testing.T) {
	t.Skip("skipped: tests are disabled for the time being")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	err := client.JobExecution.Updates(context.TODO(), schedo.JobExecutionUpdatesParams{
		Jobs: schedo.F([]string{"string"}),
	})
	if err != nil {
		var apierr *schedo.Error
		if errors.As(err, &apierr) {
